...
export function parsePiWebConfigResponse(value: unknown): PiWebConfigResponse {
  const record = requireRecord(value);
  const managedAllowedHosts = optionalManagedAllowedHosts(record["managedAllowedHosts"]);
  return {
    path: requireString(record, "path"),
...
function optionalManagedAllowedHosts(value: unknown): readonly PiWebManagedAllowedHost[] | undefined {
  if (value === undefined) return undefined;
  if (!Array.isArray(value)) throw new Error("Invalid PI WEB managedAllowedHosts field");
  return value.map((entry) => {
    const record = requireRecord(entry);
    if (record["source"] !== "safe-tunnel") {
      throw new Error("Invalid PI WEB managedAllowedHosts source");
    }
    const hostname = requireString(record, "hostname");
    if (hostname === "") throw new Error("Invalid PI WEB managedAllowedHosts hostname");
