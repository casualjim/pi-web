...
`pairedBackend.openChannel()` is the optional version-1 duplex addition. It has the same package/revision, selected-machine, project, and current-workspace authority as a direct request and does not require workspace-provider ownership. The host supplies a channel-lifetime signal and a synchronous `send()` function. Return one channel with `receive()`, optional plugin-owned `closed`, and optional `close()` members. Resolve `closed` only after the plugin's final `send()`; the host boundedly drains accepted frames in order before cleanly closing the browser transport and releasing admission. `close()` is invoked at most once for cleanup. Core validates and transports only JSON host envelopes and never interprets plugin-authored payload fields.

Channel limits are fixed host contracts: 256 KiB open input; 10-second proxy-upgrade, open, receive, close, and clean-drain deadlines; 64 KiB JSON per data frame plus host-envelope allowance; 128 queued frames and 1 MiB queued bytes in each direction at each hop; 128 admitted connections globally, 32 per plugin, and 8 per plugin/workspace at every host hop, including connections awaiting an upstream upgrade or their open frame; 12-hour maximum lifetime with no idle timeout; and a 120-byte close reason. `send()` validates and queues synchronously, so split larger output/replay into bounded frames and treat a thrown queue/validation error as channel failure.

Only text JSON is accepted. Invalid or binary frames, stale revisions or workspace scope, admission/queue overflow, callback failure or timeout, disconnect, cancellation, lifetime expiry, and shutdown abort the lifetime signal, close both directions, invoke plugin cleanup once, and release admission. The browser/server plugin never receives Fastify, a raw WebSocket, arbitrary routing, another plugin's backend, or a global event bus.
